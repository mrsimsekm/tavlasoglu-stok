    ...
    <template #body>
      <div v-if="isEmri" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div v-else class="border-2 border-green-200 bg-green-50 p-4 rounded-lg text-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-green-600 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-if="kalanTutar > 0 && tahsilatForm.tutar < kalanTutar" class="border-2 border-orange-200 bg-orange-50 p-3 rounded-lg">
          <div class="flex items-start">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-orange-600 mr-2 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
            </svg>
            <div>
              <p class="text-sm font-semibold text-orange-800">Alacak Oluşacak!</p>
              <p class="text-xs text-orange-700">
                Kalan {{ formatParaBirimi(kalanTutar - (tahsilatForm.tutar || 0)) }} tutarı için alacak kaydı oluşturulacak.
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea 
            v-model="kapanisNotlari" 
            rows="2" 
            class="form-input"
            placeholder="İş emri kapanışı ile ilgili notlar (isteğe bağlı)..."
          ></textarea>
        </div>
...
const isEmriKapat = async () => {
  ...
  if (kalanTutar.value > 0 && tahsilatForm.value.tutar > 0) {
    ...
    if (tahsilatForm.value.tutar > kalanTutar.value) {
  ...
  try {
    ...
    if (yeniKalanTutar > 0) {
      const { data: alacakData, error: alacakError } = await supabase.rpc('alacak_olustur', {
        p_is_emri_id: props.isEmri.id
      });

      if (alacakError) throw alacakError;

      if (alacakData && !alacakData.success) {
        console.error('Alacak oluşturma uyarısı:', alacakData.message);
      }
    }
