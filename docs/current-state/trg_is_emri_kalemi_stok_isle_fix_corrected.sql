...
-- Fixed the issue with parameter order in stok_azalt function calls
-- Correct order: stok_azalt(urun_id, depo_id, miktar, is_emri_id, aciklama, kullanici_id)
    ...
    IF TG_OP = 'DELETE' THEN
        ...
        ELSIF v_is_emri_rezerve_edildi = false AND OLD.kaynak_depo_id IS NOT NULL AND OLD.urun_id IS NOT NULL THEN
            PERFORM stok_arttir(
                ...
                OLD.kaynak_depo_id,
                OLD.miktar,
                OLD.is_emri_id,  -- Correct position
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        ELSIF v_is_emri_rezerve_edildi = false AND NEW.kaynak_depo_id IS NOT NULL AND NEW.urun_id IS NOT NULL THEN
            PERFORM stok_azalt(
                ...
                NEW.kaynak_depo_id,
                NEW.miktar,
                NEW.is_emri_id,  -- Correct position
